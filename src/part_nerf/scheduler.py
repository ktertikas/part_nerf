...
class LinearWarmupCosineAnnealingLR(_LRScheduler):
    ...
    Copied from https://github.com/PyTorchLightning/lightning-bolts
    """
...
def build_scheduler(config: Dict, optimizer: Optimizer) -> _LRScheduler:
    scheduler_type = config.get("type")
    warmup_steps = config.get("warmup_steps", 500)
    max_steps = config.get("max_steps")
    start_lr = config.get("start_lr", 0.0)
    eta_min = config.get("eta_min", 0.0)
    scheduler_dict = {
        "warmup_cosine": partial(
    ...
    return scheduler_dict[scheduler_type]()
