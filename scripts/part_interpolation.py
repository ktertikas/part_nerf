...
def main(args):
    ...
    print(f"Part Interpolation on device {device}")
    ...
    # Part interpolation step
    print("=====> Part Interpolation =====>")
    ...
    with torch.no_grad():
        ...
        # Part Interpolation stage
        print(f"Running part interpolation stage")
        ...
        def run_interpolation_combinations(interpolation_factor: float):
            ...
            new_part_shape_embedding = interpolate_vectors(
                part_shape_features_1, part_shape_features_2, interpolation_factor
            )
            new_part_texture_embedding = interpolate_vectors(
                part_texture_features_1, part_texture_features_2, interpolation_factor
            )
            # Only shape interpolation
            shape_predictions_interpolated = shape1_predictions.copy()
            shape_predictions_interpolated[
                "part_shape_features"
            ] = shape_predictions_interpolated["part_shape_features"].clone()
            shape_predictions_interpolated["part_shape_features"][
                :, part_ids, :
            ] = new_part_shape_embedding[:, part_ids, :]

            # Only texture interpolation
            texture_predictions_interpolated = shape1_predictions.copy()
            texture_predictions_interpolated[
                "part_texture_features"
            ] = texture_predictions_interpolated["part_texture_features"].clone()
            texture_predictions_interpolated["part_texture_features"][
                :, part_ids, :
            ] = new_part_texture_embedding[:, part_ids, :]

            # Both shape and texture interpolation
            both_predictions_interpolated = shape1_predictions.copy()
            both_predictions_interpolated[
                "part_shape_features"
            ] = both_predictions_interpolated["part_shape_features"].clone()
            both_predictions_interpolated[
                "part_texture_features"
            ] = both_predictions_interpolated["part_texture_features"].clone()
            both_predictions_interpolated["part_shape_features"][
                :, part_ids, :
            ] = new_part_shape_embedding[:, part_ids, :]
            both_predictions_interpolated["part_texture_features"][
                :, part_ids, :
            ] = new_part_texture_embedding[:, part_ids, :]
            ...
            for i in range(num_views):
                ...
                run_volumetric_rendering_from_part_features(
                    ...
                    rays_chunk,
                    embedding_id_1,
                    name=f"shape_interpolation_{interpolation_percentage}",
                )
                ...
                run_volumetric_rendering_from_part_features(
                    ...
                    rays_chunk,
                    embedding_id_1,
                    name=f"texture_interpolation_{interpolation_percentage}",
                )
                # Both
                run_volumetric_rendering_from_part_features(
                    model,
                    ...
                    rays_chunk,
                    embedding_id_1,
                    name=f"full_interpolation_{interpolation_percentage}",
                )
...
def parse_part_interpolation_args(argv):
    parser = argparse.ArgumentParser(
        description="Interpolation of one or multiple parts between two shape instances."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="The part ids that will be interpolated",
    )
    parser.add_argument(
        "--embedding_id_1",
        type=int,
    ...
    parser.add_argument(
        ...
        help="The interpolation factors used for the part interpolation",
    )
    # Experiment args
    parser.add_argument(
        "--checkpoint_id",
...
if __name__ == "__main__":
    args = parse_part_interpolation_args(sys.argv[1:])
    main(args)
